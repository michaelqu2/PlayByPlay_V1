...
class _QuestionLongDisplayPageState extends State<QuestionLongDisplayPage> {
  ...
  void createFinalSportsList() {
    ...
    List<String> physicalStrengthSports = responsesMap['physicalStrengthSports'] ?? [];
...
Ensure that all responses are concise and only contain the JSON without any additional explanations or formatting.
  ...
  void handleButtonPress() {
    for (int i = 0; i < returnResponse.length; i++) {
      if (returnResponse[i]) {
        saveImprovements(responsesMap.values.expand((x) => x).toList()[i]).then((_) {
          ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(content: Text('${responsesMap.values.expand((x) => x).toList()[i]} saved successfully!'))
          );
        }).catchError((error) {
          ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(content: Text('Failed to save ${responsesMap.values.expand((x) => x).toList()[i]}'))
          );
        });
      }
    }
        ...
        child: !isLoading
          ...
          child: Column(
            children: <Widget>[
              ...
              if (responsesMap.containsKey('finalSports') && responsesMap.containsKey('prefSports'))
                Row(
              ...
              if (responsesMap.containsKey('overallScores') && responsesMap.containsKey('psyCatScores'))
                Row(
                  mainAxisAlignment: MainAxisAlignment.spaceEvenly,
                  children: [
                    Container(
                              ...
                              RadarChartData(
                                dataSets: [
                                  RadarDataSet(
                                    ...
                                    entryRadius: 4.0,
                                    dataEntries: responsesMap['overallScores']!
                              ...
                              RadarChartData(
                                dataSets: [
                                  RadarDataSet(
                                    ...
                                    entryRadius: 4.0,
                                    dataEntries: responsesMap['psyCatScores']!
                                        .map((score) => RadarEntry(value: double.parse(score)))
                                        .toList(),
                                  ),
                                ],
              ...
              Wrap(
                ...
                children: [
                  for (String key in responsesMap.keys)
                    if (key != 'overallScores' && key != 'psyCatScores' && key != 'psyCat' && key != 'finalSports' && key != 'prefSports')
                      Container(
                        width: MediaQuery.of(context).size.width * 0.45,
                        padding: const EdgeInsets.all(16.0),
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            SizedBox(height: 8),
                            ...responsesMap[key]!.map((item) => Text(item)).toList(),
                          ],
                        ),
                      ),
                  ElevatedButton(
