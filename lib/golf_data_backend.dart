...
class _GolfDataBackendPageState extends State<GolfDataBackendPage> {
  ...
  Future<void> _predict() async {
    ...
    if (input1 == null || input2 == null || input3 == null || input4 == null ||input5 == null || input6 == null || input7 == null || input8 == null ||input9 == null || input10 == null || input11 == null) {
      setState(() {
  ...
  void processLogsForAverages(Map<String, dynamic> logs) {
    ...
    logs.forEach((key, value) {
      ...
      if (value.containsKey('Fairway Hit')) {
        fairway_hit.add((double.tryParse(value['Fairway Hit']) ?? 0.0) / 14 * 100);
      }
      if (value.containsKey('Green in regulation')) {
        green_in_regulation.add((double.tryParse(value['Green in regulation']) ?? 0.0) / 18 * 100);
      }
      if (value.containsKey('Green in Regulation on the fairway')) {
        green_in_regulation_on.add((double.tryParse(value['Green in Regulation on the fairway']) ?? 0.0) /
            (double.tryParse(value['Fairway Hit']) ?? 0.0) * 100);
      }
      if (value.containsKey('Green in Regulation on the fairway')) {
        green_in_regulation_off.add(
            ((double.tryParse(value['Green in regulation']) ?? 0.0) -
                (double.tryParse(value['Green in Regulation on the fairway']) ?? 0.0)) /
      ...
      if (value.containsKey('# of Birdies')) {
        birdie_conversion.add(
            (double.tryParse(value['# of Birdies']) ?? 0.0) / (double.tryParse(value['Green in regulation']) ?? 0.0) * 100);
      }
      if (value.containsKey('# of Putts')) {
        putts_hole.add((double.tryParse(value['# of Putts']) ?? 0.0) / (double.tryParse(value['Green in regulation']) ?? 0.0));
      }
      if (value.containsKey('# of 3 Putts')) {
        number_of_3putts.add(double.tryParse(value['# of 3 Putts']) ?? 0.0);
      }
      if (value.containsKey('Scrambling Attempts') && value.containsKey('Scrambling Success')) {
        scrambling_attempt.add((double.tryParse(value['Scrambling Success']) ?? 0.0) /
            (double.tryParse(value['Scrambling Attempts']) ?? 0.0) *
            100);
      }
      if (value.containsKey('Scrambling Attempts in Bunker') && value.containsKey('Scrambling Success in Bunker')) {
        scrambling_attempt_bunker.add((double.tryParse(value['Scrambling Success in Bunker']) ?? 0.0) /
            (double.tryParse(value['Scrambling Attempts in Bunker']) ?? 0.0) *
            100);
      }
      if (value.containsKey('Scrambling Attempts') &&
        ...
        scrambling_attempt_rough.add(((double.tryParse(value['Scrambling Success']) ?? 0.0) -
    ...
    setState(() {
      averageScore = scores.isNotEmpty ? (scores.reduce((a, b) => a + b) / scores.length).toStringAsFixed(2) : 'No data';
      averageDistance = drive_distances.isNotEmpty
          ? (drive_distances.reduce((a, b) => a + b) / drive_distances.length).toStringAsFixed(2)
          : 'No data';
      fairwayAvg = fairway_hit.isNotEmpty ? (fairway_hit.reduce((a, b) => a + b) / fairway_hit.length).toStringAsFixed(2) : 'No data';
      greenInRegAvg = green_in_regulation.isNotEmpty
          ? (green_in_regulation.reduce((a, b) => a + b) / green_in_regulation.length).toStringAsFixed(2)
          : 'No data';
      greenInRegOnFairwayAvg = green_in_regulation_on.isNotEmpty
          ? (green_in_regulation_on.reduce((a, b) => a + b) / green_in_regulation_on.length).toStringAsFixed(2)
          : 'No data';
      greenInRegOffAvg = green_in_regulation_off.isNotEmpty
          ? (green_in_regulation_off.reduce((a, b) => a + b) / green_in_regulation_off.length).toStringAsFixed(2)
          : 'No data';
      birdieConversionAvg = birdie_conversion.isNotEmpty
          ? (birdie_conversion.reduce((a, b) => a + b) / birdie_conversion.length).toStringAsFixed(2)
          : 'No data';
      puttsHoleAvg = putts_hole.isNotEmpty
          ? (putts_hole.reduce((a, b) => a + b) / putts_hole.length).toStringAsFixed(2)
          : 'No data';
      puttsAvg = number_of_putts.isNotEmpty
          ? (number_of_putts.reduce((a, b) => a + b) / number_of_putts.length).toStringAsFixed(2)
          : 'No data';
      threePuttsAvg = number_of_3putts.isNotEmpty
          ? (number_of_3putts.reduce((a, b) => a + b) / number_of_3putts.length).toStringAsFixed(2)
          : 'No data';
      scramblingAttemptAvg = scrambling_attempt.isNotEmpty
          ? (scrambling_attempt.reduce((a, b) => a + b) / scrambling_attempt.length).toStringAsFixed(2)
          : 'No data';
      scramblingAttemptBunkerAvg = scrambling_attempt_bunker.isNotEmpty
          ? (scrambling_attempt_bunker.reduce((a, b) => a + b) / scrambling_attempt_bunker.length).toStringAsFixed(2)
          : 'No data';
      scramblingAttemptRoughAvg = scrambling_attempt_rough.isNotEmpty
          ? (scrambling_attempt_rough.reduce((a, b) => a + b) / scrambling_attempt_rough.length).toStringAsFixed(2)
          : 'No data';
    });
  }
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: 20),
              Text('Average Score: $averageScore', style: TextStyle(fontSize: 15)),
              SizedBox(height: 10),
              Text('Average Drive Distance: $averageDistance', style: TextStyle(fontSize: 15)),
              SizedBox(height: 10),
              Text('Average Fairway Hit: $fairwayAvg', style: TextStyle(fontSize: 15)),
              SizedBox(height: 10),
              Text('Average Green in Regulation: $greenInRegAvg', style: TextStyle(fontSize: 15)),
              SizedBox(height: 10),
              Text('Average Green in Regulation on Fairway: $greenInRegOnFairwayAvg', style: TextStyle(fontSize: 15)),
              SizedBox(height: 10),
              Text('Average Green in Regulation Off Fairway: $greenInRegOffAvg', style: TextStyle(fontSize: 15)),
              SizedBox(height: 10),
              Text('Average Birdie Conversion Rate: $birdieConversionAvg', style: TextStyle(fontSize: 15)),
              SizedBox(height: 10),
              Text('Average Putts per Hole: $puttsHoleAvg', style: TextStyle(fontSize: 15)),
              SizedBox(height: 10),
              Text('Average Number of Putts: $puttsAvg', style: TextStyle(fontSize: 15)),
              SizedBox(height: 10),
              Text('Average Number of 3 Putts: $threePuttsAvg', style: TextStyle(fontSize: 15)),
              SizedBox(height: 10),
              Text('Average Scrambling Attempt: $scramblingAttemptAvg', style: TextStyle(fontSize: 15)),
              SizedBox(height: 10),
              Text('Average Scrambling Attempt in Bunker: $scramblingAttemptBunkerAvg', style: TextStyle(fontSize: 15)),
              SizedBox(height: 10),
              Text('Average Scrambling from Rough: $scramblingAttemptRoughAvg', style: TextStyle(fontSize: 15)),
              const SizedBox(height: 20),
              logs.isNotEmpty
                  ? ListView.separated(
                shrinkWrap: true,
                ...
                itemBuilder: (context, index) {
                  return Card(
                    child: ListTile(
                      title: Text('Date: ${logs[index]["logDate"]}'),
                      subtitle: Row(
                          children: [
                            Text('${logs[index]["Scrambling Attempts"]}',
                              style: TextStyle(
                                fontSize: 9,
                              ),
                            ),
                            Text(' ${logs[index]["Scrambling Success"]}',
                              style: TextStyle(
                                fontSize: 9,
                              ),
                            ),
                            Text(' ${logs[index]["Scrambling Attempts in Bunker"]}',
                              style: TextStyle(
                                fontSize: 9,
                              ),
                            ),
                            Text(' ${logs[index]["Scrambling Success in Bunker"]}',
                              style: TextStyle(
                                fontSize: 9,
                              ),
                            ),
